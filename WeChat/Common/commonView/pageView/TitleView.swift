...
class TitleView: UIView {
    ...
    // MARK: 存储属性
    fileprivate lazy var titleLabels : [UIButton] = [UIButton]()
...
//        addSubview(splitLineView)
    ...
    fileprivate func setupTitleLabels() {
        for (index, title) in titles.enumerated() {
            ...
            btn.backgroundColor = index == 0 ? style.selectedColor : style.normalColor

            btn.addTarget(self, action:#selector(titleLabelClick(_ :)), for: UIControl.Event.touchUpInside)
            titleLabels.append(btn)
            
            scrollView.addSubview(btn)
            
            
            let viewH = 1/UIScreen.main.scale
            let viewLine1 = UIView(frame: CGRect(x: 0 + CGFloat(index)*kEmoticonWidth, y: 5, width: viewH, height: frame.height-10))
        ...
        let viewH = 1/UIScreen.main.scale
        let viewLine1 = UIView(frame: CGRect(x: style.titleMargin*CGFloat(titles.count) + CGFloat(titles.count)*kEmoticonWidth, y: 5, width: viewH, height: frame.height-10))
        viewLine1.backgroundColor = UIColor.Gray213Color()
        scrollView.addSubview(viewLine1)
    }
    
    
    fileprivate func setupTitleButtonsPosition() {
...
// MARK:- 事件处理
extension TitleView {
    @objc fileprivate func titleLabelClick(_ tap : UIButton) {
        // 0.获取当前Label
        let currentLabel = tap 
        ...
        // 3.切换文字的颜色
        currentLabel.backgroundColor = style.selectedColor
        oldLabel.backgroundColor = style.normalColor
...
//
//        // 3.2.变化sourceLabel
//        sourceLabel.backgroundColor = UIColor(r: selectedColorRGB.0 - colorDelta.0 * progress, g: selectedColorRGB.1 - colorDelta.1 * progress, b: selectedColorRGB.2 - colorDelta.2 * progress)
//
//        // 3.2.变化targetLabel
//        targetLabel.backgroundColor = UIColor(r: normalColorRGB.0 + colorDelta.0 * progress, g: normalColorRGB.1 + colorDelta.1 * progress, b: normalColorRGB.2 + colorDelta.2 * progress)
        
        
        sourceLabel.backgroundColor = style.normalColor
        targetLabel.backgroundColor = style.selectedColor
        // 4.记录最新的index
    ...
    func contentViewDidEndScroll() {
        ...
        var maxOffset = scrollView.contentSize.width - bounds.width
        if maxOffset < 0 {
            maxOffset = 0
        }
